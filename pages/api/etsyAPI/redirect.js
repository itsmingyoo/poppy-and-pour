...
async function handler(req, res) {
    ...
    const uriResponse = await fetch(redirectUri)
    if(uriResponse.ok) {
        const data = uriResponse.json()
        console.log("DATA BEFORE PARSE ", data)
        const {uri, clientVerifier} = data
        console.log("AUTH CODEEEEEEEEEEEEEEEEE ----> ", uri)
        console.log('CLIENT VERIFYER ---> ', clientVerifier)
    } else {
